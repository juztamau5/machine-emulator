...
enum MODE_shifts : uint8_t { ACCESS_MODE_PRV_SHIFT = 0, ACCESS_MODE_VRT_SHIFT = 2 };

/// \brief Access mode masks
enum MODE_masks : uint8_t {
    ACCESS_MODE_PRV_MASK = 3 << ACCESS_MODE_PRV_SHIFT,
    ACCESS_MODE_VRT_MASK = 1 << ACCESS_MODE_VRT_SHIFT
};

/// \brief Access modes
enum MODE_constants : uint8_t {
    MODE_U = PRV_U,
    MODE_HS = PRV_S,
    MODE_M = PRV_M,
    MODE_VS = ACCESS_MODE_VRT_MASK | PRV_S,
    MODE_VU = ACCESS_MODE_VRT_MASK | PRV_U
};
...
enum XIE_XIP_STANDARD_BITS_MASK : uint64_t {
    STANDARD_BITS_MASK = (UINT64_C(1) << 16) - 1
};
...
enum MIX_RW_masks : uint64_t {
    ...
    MIE_W_MASK = MIP_MSIP_MASK | MIP_MTIP_MASK | MIP_MEIP_MASK | MIP_SSIP_MASK | MIP_STIP_MASK | MIP_SEIP_MASK | MIE_VIRTUAL_RW_MASK,
...
enum IFLAGS_masks : uint64_t {
    ...
    IFLAGS_VRT_MASK = UINT64_C(1) << IFLAGS_VRT_SHIFT,
};
...
enum CARTESI_init : uint64_t {
    ...
    MISA_INIT = (MISA_MXL_VALUE << MISA_MXL_SHIFT) | MISA_EXT_S_MASK | MISA_EXT_U_MASK | MISA_EXT_I_MASK |
        MISA_EXT_M_MASK | MISA_EXT_A_MASK | MISA_EXT_H_MASK | MISA_EXT_F_MASK | MISA_EXT_D_MASK |
