...
static uint8_t translate_virtual_address(STATE_ACCESS &a, uint64_t *ppaddr, uint64_t vaddr, MODE_constants access_mode,
    int xwr_shift) {
    const uint8_t priv = (access_mode & ACCESS_MODE_PRV_MASK) >> ACCESS_MODE_PRV_SHIFT;
    const uint8_t virt = (access_mode & ACCESS_MODE_VRT_MASK) >> ACCESS_MODE_VRT_SHIFT;
