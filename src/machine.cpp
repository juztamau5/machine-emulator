...
bool machine::read_iflags_VRT(void) const {
    return m_s.iflags.VRT;
}

void machine::reset_iflags_VRT(void) {
    m_s.iflags.VRT = false;
}
...
static MODE_constants get_access_mode(uint8_t priv, bool virt, uint64_t mstatus) {
...
void machine::read_virtual_memory(uint64_t vaddr_start, unsigned char *data, uint64_t length) {
    ...
    for (uint64_t vaddr_page = vaddr_page_start; vaddr_page < vaddr_page_limit; vaddr_page += PMA_PAGE_SIZE) {
        ...
        uint8_t cause = translate_virtual_address<state_access, ACCESS_TYPE_LOAD, false>(a, &paddr_page, vaddr_page,
            access_mode, PTE_XWR_R_SHIFT);
        if (cause) {
...
void machine::write_virtual_memory(uint64_t vaddr_start, const unsigned char *data, size_t length) {
    ...
    for (uint64_t vaddr_page = vaddr_page_start; vaddr_page < vaddr_page_limit; vaddr_page += PMA_PAGE_SIZE) {
        ...
        uint8_t cause = translate_virtual_address<state_access, ACCESS_TYPE_STORE, false>(a, &paddr_page, vaddr_page,
            access_mode, PTE_XWR_R_SHIFT);
        if (cause) {
            throw std::runtime_error{"page fault"};
        }
